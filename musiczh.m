...
function [basicf,oneortwo] = harmonicwave( x,fs )%谐波分析法
    xx=fft(x);
    m=abs(xx);
    N=length(m);
    n=0;
    ...
    for i=1:5
        l=n/i;
        B=m(floor(l),1)+m(floor(2*l),1)+m(floor(3*l),1)+m(floor(4*l),1)+m(floor(5*l),1)+m(ceil(l),1)+m(ceil(2*l),1)+m(ceil(3*l),1)+m(ceil(4*l),1)+m(ceil(5*l),1);
        if B>b
            b=B;
            num=i;
        end
...
function f = fre1( f0,x,fs )
    Wp=f0*2/fs;Ws=2*f0*2/fs;Rp=3;Rs=20;
